...
/*
    SECCIÓN PROBLEMAS
      - Programar un algoritmo para cada método y que este retorne lo requerido.
...
function sortPaddockTypeByTotalArea() {
  // CODE HERE
  const sortedPaddockNames = paddockType
    .map((type) => ({
      name: type.name,
      area: paddocks
        .filter((paddock) => paddock.paddockTypeId === type.id)
        .reduce((sum, paddock) => sum + paddock.area, 0),
    }))
    .sort((a, b) => b.area - a.area)
    .map((type) => type.name);
  return sortedPaddockNames;
}
